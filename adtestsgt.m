function [H, pVal, ADStat, CV] = adtestsgt(x, varargin)
...
paramNames = {'Distribution', 'Alpha', 'MCTol', 'Asymptotic', 'Parmhat'};
dflts  =     {'normal' ,       0.05,     [],       false, [3,6,0,mean(x),std(x)]};

[valDistr, valAlpha, valMCTol, valAsymptotic, valParmhat] = ...
...
if internal.stats.isString(valDistr)
    DistrNames = {'normal', 'exponential', 'ev', 'lognormal', 'weibull', 'extreme value', 'sgt'};
...
if ~isCompositeTest
    ...
    z = sgtcdf(x, Param, parmhat(4), parmhat(5));
    ...
    switch Distr
        ...
        case 'sgt'
            z = sgtcdf(x, Param, Parmhat(4), Parmhat(5)); %MLE estimate
            ADStat = ComputeADStat(z,n);
    end
    
    if isempty(mctol)
        switch Distr
            %assuming the decresing of leptokurtosis so that the sgt for
            %big sample observations tends to a normal distribution
            case {'normal','sgt'}, [alphas, CVs] = computeCriticalValues_norm(n);
            case 'exponential', [alphas, CVs] = computeCriticalValues_exp(n);
            case {'ev', 'extreme value'}, [alphas, CVs] = computeCriticalValues_ev(n);
            %case 'sgt', [alphas, CVs] = adtestMC(ADStat, n, alpha, Distr, 0.01,Parmhat);
        end
...
%------------------------------------------
function [crit, p] = adtestMC(ADStat, n, alpha, distr, mctol,Parmhat)
...
while true
    ...
    switch distr
        ...
        case 'sgt'
            ...
            Param = [Parmhat(1),Parmhat(2),Parmhat(3)];
            for rep = 1:length(ADstatMC)
                x = sgtrnd(Param, Parmhat(4), Parmhat(5),n);
                %pHat = sgtfit(x);
                xCDF = sort(x);
                nullCDF = sgtcdf(xCDF,Param, Parmhat(4), Parmhat(5));
                w = 2*(1:n) - 1 ;
                ADstatMC(rep) = -w*(log(nullCDF)+ log(1-nullCDF(end:-1:1)))/n - n;
            end
    end
